...
from .utils import (
    get_kakao_user_info, get_google_user_info,
    get_access_token_from_code, normalize_profile_img,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if code:
            try:
                ...
                # 발급 실패 시 에러 응답
                return Response({'detail': '소셜 엑세스 토큰 발급 실패', 'error': str(e)}, status=400)
        ...
        user = User.objects.filter(social_type=provider.upper(), social_id=social_id).first()
        # 블랙리스트 유저일 경우 로그인 거부
        if user:
            bl = UserBlacklist.objects.filter(user=user, is_active=True).first()
            if bl:
                return Response({'detail': '블랙리스트 계정', 'reason': bl.reason}, status=403)
            # 탈퇴/비활성 유저 거부
